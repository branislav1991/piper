...
def test_sample_beta():
    ...
    assert jnp.isclose(jnp.mean(samples), 0.45465213)
    ...
    assert jnp.isclose(jnp.mean(samples), 0.2994858)
...
def test_sample_beta_flexible_one_node():
    ...
    assert jnp.allclose(sample, jnp.array([0.37926483, 0.9784184]))
...
def test_kl_beta_beta_one_dimensional():
    ...
    assert jnp.isclose(func.compute_kl_div(model, 'n1', 'n2'), 0.6137059)
...
def test_kl_beta_beta_multi_dimensional():
    ...
    assert jnp.allclose(func.compute_kl_div(model, 'n1', 'n2'),
...
def test_log_prob_beta():
    ...
    assert (jnp.isclose(log_prob_1, 0.05924129))
    assert (jnp.isclose(log_prob_2, 0.05924118))
