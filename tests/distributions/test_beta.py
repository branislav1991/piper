...
def test_sample_beta():
    ...
    model = dist.beta(model, 'n', jnp.array([0.5]), jnp.array([0.5]))
    ...
    model = models.create_forward_model()
    model = dist.beta(model, 'n', jnp.array([2.]), jnp.array([5.]))
    samples = jax.vmap(lambda k, m: m.sample(k)['n'],
                       in_axes=(0, None),
                       out_axes=0)(keys, model)
    ...
    model = models.create_forward_model()
    model = dist.beta(model, 'n', jnp.full((2, 2), 0.5), jnp.full((2, 2), 0.5))
...
def test_sample_beta_flexible_one_node():
    # test with one node
    model = models.create_forward_model()
    model = dist.beta(model, 'n', jnp.array([0.5, 0.5]),
...
def test_kl_beta_beta_one_dimensional():
    model = models.create_forward_model()
    model = dist.beta(model, 'n1', jnp.array(1.0), jnp.array(0.5))
    model = dist.beta(model, 'n2', jnp.array(2.0), jnp.array(0.5))
...
def test_kl_beta_beta_multi_dimensional():
    model = models.create_forward_model()
    model = dist.beta(model, 'n1', jnp.array([1.0, 2.0]), jnp.array([0.5,
                                                                     0.5]))
    model = dist.beta(model, 'n2', jnp.array([2.0, 1.0]), jnp.array([0.5,
                                                                     0.5]))
...
def test_sample_conditioned():
    model = models.create_forward_model()
    model = dist.beta(model, 'n1', jnp.array([0.5]), jnp.array([0.5]))
